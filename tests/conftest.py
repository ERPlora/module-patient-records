"""Pytest fixtures for patient_records module tests."""
import uuid
import pytest
...
@pytest.fixture
def patient_record(db, hub_id):
    """Create a test PatientRecord."""
    return PatientRecord.objects.create(
        hub_id=hub_id,
        patient_name='Test Patient Name',
        date_of_birth=timezone.now().date(),
        gender='Test Gender',
        blood_type='Test Blood Type',
        allergies='Test description',
        medical_notes='Test description',
    )


@pytest.fixture
def treatment(db, hub_id):
